 ...
 */
class CopiaIncidenciaEnEventoManager
{
    private $array = array();
     ...
     * 
     * @param string $name
     * @param array $tsol one element in name tsol
     * @param array $clientes
     * @param array $traducciones
     * @param array $matriculas
     * @param string $modo
     */
    public function __construct($name, $tsol, $clientes, $traducciones = null, $matriculas = null, $modo = null)
    {
     ...
     * A partir de la version 1.9 ya no se usa
     */
...
//        $pattern = "/^(\[(\w+)*\]){3}/";
        ...
        if (method_exists($incidencia, 'getTitulo') and null != $incidencia->getTitulo())
        {
            ...
            if ($result == 1 and count($matches) == 2)
            {
                if (in_array(strtoupper($matches[1]), $this->confCliente))
                {
                    $this->cliente = strtolower($matches[1]);
                }
                else
                {
                    $result = 0;
    ...
    {
        $tecnico = "";
        if (method_exists($incidencia, 'getTecnicoAsignadoInicial') and null != $incidencia->getTecnicoAsignadoInicial())
        {
            $tecnico = $incidencia->getTecnicoAsignadoInicial();
        }
        elseif (method_exists($incidencia, 'getTecnicoAsignadoFinal') and null != $incidencia->getTecnicoAsignadoFinal())
        {
            $tecnico = $incidencia->getTecnicoAsignadoFinal();
        }
        if ($tecnico != "" and is_string($tecnico))
        {
            if (is_array($this->matriculas) and count($this->matriculas) > 0)
            {
                if (array_key_exists(strtoupper($tecnico), $this->matriculas))
                {
                    $this->tecnico = strtolower($this->matriculas[$tecnico]);
                }
                else
                {
                    $this->tecnico = strtolower($tecnico);
                }
            }
            else
            {
...
//            
        ...
        else
        {
            ...
            if ($this->tecnico == "ficticio" and method_exists($incidencia, 'getGrupoDestino') and null != $incidencia->getGrupoDestino())
            {
                $this->tecnico = $incidencia->getGrupoDestino();
            }
        }
    ...
    {
        //Hook para determinar la plantilla
        $resoluciones = array('RESOLUCION_TICKET', 'RESOLUCION_ALARMA', 'RESOLUCION');
        $case = "";
        if (in_array($this->platillaName, $resoluciones))
        {
            $case = 'RESOLUCION';
        }
        else
        {
            $case = $this->platillaName;
        }
        switch ($case)
        {
            case 'RESOLUCION':
        ...
        if (strlen($this->detalle) > 250 and $this->detalle != 'missing')
        {
            $this->detalle = substr($this->detalle, 0, 250);
        }
        $this->array['detalle'] = trim($this->translateChars($this->detalle));
    }
    ...
    protected function translateChars($s)
    {
        $replace = array(
            ...
            'Š' => 'S', 'š' => 's', 'Ş' => 'S', 'ș' => 's', 'Ș' => 'S', 'ş' => 's', 'ß' => 'ss', 'Ś' => 'S', 'ś' => 's', '§' => 's',
//            '$' => 'S',
            'ț' => 't', 'Ț' => 'T',
            'Ù' => 'U', 'Ú' => 'U', 'Û' => 'U', 'Ü' => 'Ue',
            'ù' => 'u', 'ú' => 'u', 'û' => 'u', 'ü' => 'ue',
